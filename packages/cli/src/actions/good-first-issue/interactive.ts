...
import type { GoodFirstIssueSuggestion, GoodFirstIssueResults } from '@cezar/core';
    ...
    if (stopped) {
      ...
      for (const suggestion of this.results.suggestions) {
        if (!reviewed.has(suggestion.number)) {
          this.results.store.setAnalysis(suggestion.number, { goodFirstIssueAnalyzedAt: null });
        }
      }
    }
